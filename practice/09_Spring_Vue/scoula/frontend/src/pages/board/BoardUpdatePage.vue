...
const reset = () => {
  article.no = orgAtrticle.value.no;
  article.title = orgAtrticle.value.title;
  article.writer = orgAtrticle.value.writer;
  article.content = orgAtrticle.value.content;
  console.log(article);
};

const load = async () => {
  const data = await boardApi.get(no);
  orgAtrticle.value = { ...data };
